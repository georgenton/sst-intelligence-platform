...
async function provisionDemoSession(page: Page, suffix: number) {
  ...
  await page.getByLabel('Nombre').fill('Profesional Metodologías E2E');
  await page.getByLabel('Correo').fill(`risk-methods-e2e-${suffix}@example.test`);
  await page.getByLabel('Contraseña').fill('risk-methods-e2e-password-123');
