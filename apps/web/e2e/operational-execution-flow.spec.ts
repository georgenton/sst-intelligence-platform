...
test('obligación visible durante setup y permiso demo con aprobación separada', async ({
  ...
  await page.getByLabel('Rol').selectOption({ label: 'Responsable SST' });
  await page.getByRole('button', { name: 'Crear invitación' }).click();
  const invitationLink = await page.getByLabel('Enlace de invitación').inputValue();
  expect(invitationLink).toContain('/invite/accept#token=');

  await page.getByRole('button', { name: 'Salir' }).click();
  await page.goto(invitationLink);
  await page.getByRole('main').getByRole('link', { name: 'Crear cuenta' }).click();
  await page.getByLabel('Nombre').fill('Responsable SST E2E');
  await page.getByLabel('Correo').fill(managerEmail);
  await page.getByLabel('Contraseña').fill(password);
  await page.getByRole('button', { name: 'Crear cuenta' }).click();
  await expect(page.getByRole('heading', { name: 'Revisa tu invitación' })).toBeVisible();
  await expect(page.getByText(organizationName)).toBeVisible();
  await page.getByRole('button', { name: 'Aceptar invitación' }).click();
  await expect(page.getByRole('heading', { name: 'Centro de comando' })).toBeVisible();
  await page.getByRole('link', { name: 'Equipo', exact: true }).click();
  await expect(page.getByText('Responsable SST E2E')).toBeVisible();
  ...
  await page
    .getByLabel('Persona aprobadora')
    .selectOption({ label: 'Responsable SST E2E · Responsable SST' });
