...
export class AdaptiveConfigurationService {
  constructor(
    ...
    @Inject(ADAPTIVE_SESSION_MUTATION_SYNC)
    private readonly mutationSync: AdaptiveSessionMutationSync,
  ) {}

  listRulePacks() {
    return this.prisma.adaptiveRulePackVersion.findMany({
      where: { isDemo: true, regulatory: false, sealedAt: { not: null } },
      select: {
        id: true,
        version: true,
  ...
  async createSession(
    ...
    const created = await this.serializable(async (transaction) => {
      ...
      const centers = selectedIds ? allCenters.filter(({ id }) => selectedIds.has(id)) : allCenters;
  ...
  async submitAnswers(
    ...
    const revision = await this.serializable(async (transaction) => {
      ...
      const scopes = new Map(session.scopes.map((scope) => [scope.id, scope]));
      const facts = new Map(
      ...
      for (const answer of input.answers) {
        const scope = scopes.get(answer.scopeId);
        if (!scope) throw new ForbiddenException('Alcance no válido.');
        ...
        if (factVersion.factDefinition.defaultScope !== scope.kind)
          throw new ForbiddenException('El hecho no pertenece al tipo de alcance indicado.');
        if (
  ...
  async evaluate(
    ...
    const created = await this.serializable(async (transaction) => {
      ...
      if (runNumber > ADAPTIVE_LIMITS.evaluationRunsPerSession)
      ...
      const facts: AdaptiveFactInput[] = session.answers.map((answer) => ({
        ...
        factVersionId: answer.factVersionId,
        source: answer.source,
        value: answer.typedValue as AdaptiveFactValue,
      }));
      let evaluation: ReturnType<typeof evaluateAdaptiveConfiguration>;
      try {
        evaluation = evaluateAdaptiveConfiguration({
          pack,
          scopes,
          facts,
          auditContext: {
            packVersionId: session.rulePackVersionId,
            packContentHash: session.rulePackVersion.contentHash,
          },
        });
      } catch (error) {
        if (error instanceof AdaptiveLimitExceededError)
          throw new BadRequestException({ code: error.code, limit: error.limit });
        throw error;
      }
      ...
      await transaction.adaptiveConfigurationSession.update({
        where: { id: session.id },
        data: {
          status: evaluation.questions.length === 0 ? 'READY_TO_PROPOSE' : 'COLLECTING_INFORMATION',
        },
      });
      return {
        runId: run.id,
    ...
    try {
      ...
      if ((error as { code?: string }).code === 'P2034')
        throw new ConflictException({
          code: 'ADAPTIVE_SESSION_CONFLICT',
          message: 'La sesión cambió durante la operación. Recarga antes de continuar.',
        });
      throw error;
    }
  }
