...
export class OperationalSearchService {
  ...
  async search(organizationId: string, query: OperationalSearchQueryDto) {
    const requestedTypes = query.types?.length ? query.types : [...operationalSearchTypes];
    const effective = await this.entitlements.effective(organizationId);
    const featureByType: Partial<Record<(typeof operationalSearchTypes)[number], string>> = {
      INSPECTION: 'module.inspections',
      FINDING: 'module.inspections',
      ACTION: 'module.inspections',
      SAFETY_OBSERVATION: INCIDENTS_FEATURE_KEY,
      INCIDENT: INCIDENTS_FEATURE_KEY,
      TRAINING: TRAINING_FEATURE_KEY,
      PPE: PPE_FEATURE_KEY,
    };
    const types = requestedTypes.filter((type) => {
      const feature = featureByType[type as (typeof operationalSearchTypes)[number]];
      return !feature || effective.features[feature] === true;
    });
    if (types.length === 0) {
    ...
    const branches: Partial<Record<(typeof operationalSearchTypes)[number], Prisma.Sql>> = {
      INSPECTION: Prisma.sql`SELECT i.id, 'INSPECTION'::text AS type, i.title,
      ...
      FINDING: Prisma.sql`SELECT f.id, 'FINDING', f.title, left(f.description,240), f.status::text, f."workCenterId", wc.name, f."updatedAt",
      ...
      SAFETY_OBSERVATION: Prisma.sql`SELECT o.id, 'SAFETY_OBSERVATION', o.title, left(o.description,240), o.status::text, o."workCenterId", wc.name, o."updatedAt",
      ...
      INCIDENT: Prisma.sql`SELECT n.id, 'INCIDENT', n.title, 'Registro de incidente · detalle restringido', n.status::text, n."workCenterId", wc.name, n."updatedAt",
      ...
      ACTION: Prisma.sql`SELECT a.id, 'ACTION', a.title, left(coalesce(a.description,'Acción correctiva'),240), a.status::text, f."workCenterId", wc.name, a."updatedAt",
      ...
      PLAN_ITEM: Prisma.sql`SELECT p.id, 'PLAN_ITEM', p.title, left(coalesce(p.description,'Ítem de plan'),240), coalesce(e.status::text,'PLANNED'), p."workCenterId", wc.name, p."createdAt",
      ...
      WORKER: Prisma.sql`SELECT w.id, 'WORKER', w."displayName", left(coalesce(w."internalCode",'') || CASE WHEN w."jobTitle" IS NULL THEN '' ELSE ' · ' || w."jobTitle" END,240), w.status::text, w."workCenterId", wc.name, w."updatedAt",
      ...
      POSITION: Prisma.sql`SELECT p.id, 'POSITION', p.name, left(coalesce(p.description,p.code,'Cargo'),240), CASE WHEN p."isActive" THEN 'ACTIVE' ELSE 'INACTIVE' END, NULL::uuid, NULL::text, p."updatedAt",
      ...
      TRAINING: Prisma.sql`SELECT t.id, 'TRAINING', t.title, left(coalesce(t.description,t.category),240), CASE WHEN t."isActive" THEN 'ACTIVE' ELSE 'INACTIVE' END, NULL::uuid, NULL::text, t."updatedAt",
      ...
      PPE: Prisma.sql`SELECT p.id, 'PPE', p.name, left(coalesce(p.description,p.category::text),240), p.status::text, NULL::uuid, NULL::text, p."updatedAt",
      ...
      WORK_CENTER: Prisma.sql`SELECT wc.id, 'WORK_CENTER', wc.name, left(coalesce(wc.city,'Centro de trabajo'),240), CASE WHEN wc."isActive" THEN 'ACTIVE' ELSE 'INACTIVE' END, wc.id, wc.name, wc."updatedAt",
    ...
    const authorizedBranches = types.map(
      (type) => branches[type as (typeof operationalSearchTypes)[number]]!,
    );
    const rows = await this.prisma.$queryRaw<SearchRow[]>(Prisma.sql`
      ...
      candidates(id, type, title, snippet, status, "workCenterId", "workCenterName", "occurredAt", rank)
      AS (${Prisma.join(authorizedBranches, ' UNION ALL ')})
      SELECT *, count(*) OVER()::int AS total FROM candidates
      ORDER BY 9 DESC, 8 DESC, 2 ASC, 1 ASC
    ...
    return {
      items: rows.map(({ rank: _rank, total: _total, ...row }) => ({
        ...
        navigationKind: ['INSPECTION', 'SAFETY_OBSERVATION', 'INCIDENT', 'WORKER'].includes(
          row.type,
        )
          ? 'DETAIL'
          : 'MODULE',
        ctaLabel: ['INSPECTION', 'SAFETY_OBSERVATION', 'INCIDENT', 'WORKER'].includes(row.type)
          ? 'Abrir registro'
          : 'Abrir módulo',
      })),
