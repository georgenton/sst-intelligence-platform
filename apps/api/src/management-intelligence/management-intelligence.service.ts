...
export class ManagementIntelligenceService {
  ...
  async summary(organizationId: string, query: ManagementIntelligenceQueryDto) {
    ...
    const findingCategory = query.findingCategory ?? null;
    const observationCategory = query.observationCategory ?? null;
    const incidentEventType = query.incidentEventType ?? null;
    const from = query.dateFrom ? new Date(query.dateFrom) : null;
    const to = query.dateTo ? new Date(query.dateTo) : null;
    const effective = await this.entitlements.effective(organizationId);
    const inspectionsEnabled = effective.features['module.inspections'] === true;
    const incidentsEnabled = effective.features[INCIDENTS_FEATURE_KEY] === true;
    const trainingEnabled = effective.features[TRAINING_FEATURE_KEY] === true;
    const ppeEnabled = effective.features[PPE_FEATURE_KEY] === true;
    const eventBranches: Prisma.Sql[] = [];
    if (incidentsEnabled) {
      eventBranches.push(
        Prisma.sql`SELECT "observedAt" AS event_at,'SAFETY_OBSERVATION' AS type FROM "SafetyObservation" WHERE "organizationId"=${organizationId}::uuid AND (${centerId}::uuid IS NULL OR "workCenterId"=${centerId}::uuid) AND (${observationCategory}::text IS NULL OR category::text=${observationCategory})`,
        Prisma.sql`SELECT "occurredAt" AS event_at,'INCIDENT' AS type FROM "Incident" WHERE "organizationId"=${organizationId}::uuid AND (${centerId}::uuid IS NULL OR "workCenterId"=${centerId}::uuid) AND (${incidentEventType}::text IS NULL OR "eventType"::text=${incidentEventType})`,
      );
    }
    if (inspectionsEnabled) {
      eventBranches.push(
        Prisma.sql`SELECT "createdAt" AS event_at,'FINDING' AS type FROM "InspectionFinding" WHERE "organizationId"=${organizationId}::uuid AND (${centerId}::uuid IS NULL OR "workCenterId"=${centerId}::uuid) AND (${findingCategory}::text IS NULL OR category=${findingCategory})`,
      );
    }
    const trendPromise = eventBranches.length
      ? this.prisma.$queryRaw<Array<{ month: Date; type: string; count: number }>>(
          Prisma.sql`SELECT date_trunc('month', event_at) AS "month", type, count(*)::int AS count FROM (${Prisma.join(eventBranches, ' UNION ALL ')}) events WHERE (${from}::timestamp IS NULL OR event_at>=${from}) AND (${to}::timestamp IS NULL OR event_at<${to}) GROUP BY 1,2 ORDER BY 1,2`,
        )
      : Promise.resolve([]);
    const [
      findings,
      ...
      inspectionsEnabled
        ? this.prisma.$queryRaw<MethodRow[]>(
          ...
          WHERE f."organizationId"=${organizationId}::uuid AND (${centerId}::uuid IS NULL OR f."workCenterId"=${centerId}::uuid) AND (${findingCategory}::text IS NULL OR f.category=${findingCategory}) AND (${from}::timestamp IS NULL OR f."createdAt">=${from}) AND (${to}::timestamp IS NULL OR f."createdAt"<${to})
          GROUP BY f."riskMethodVersionId"
          UNION ALL
          SELECT f."residualMethodVersionId" AS id, 0::int, count(*)::int
          FROM "InspectionFinding" f
          WHERE f."organizationId"=${organizationId}::uuid AND f."residualMethodVersionId" IS NOT NULL AND (${centerId}::uuid IS NULL OR f."workCenterId"=${centerId}::uuid) AND (${findingCategory}::text IS NULL OR f.category=${findingCategory}) AND (${from}::timestamp IS NULL OR f."createdAt">=${from}) AND (${to}::timestamp IS NULL OR f."createdAt"<${to})
      ...
      inspectionsEnabled
        ? this.prisma.$queryRaw<ResidualRow[]>(
            Prisma.sql`SELECT f."residualMethodVersionId" AS "methodVersionId", f."residualRiskLevel"::text AS level, count(*)::int AS count FROM "InspectionFinding" f WHERE f."organizationId"=${organizationId}::uuid AND f."residualMethodVersionId" IS NOT NULL AND f."residualRiskLevel" IS NOT NULL AND (${centerId}::uuid IS NULL OR f."workCenterId"=${centerId}::uuid) AND (${findingCategory}::text IS NULL OR f.category=${findingCategory}) AND (${from}::timestamp IS NULL OR f."createdAt">=${from}) AND (${to}::timestamp IS NULL OR f."createdAt"<${to}) GROUP BY f."residualMethodVersionId",f."residualRiskLevel" ORDER BY f."residualMethodVersionId",f."residualRiskLevel"`,
          )
        : [],
      trendPromise,
    ]);
    return {
      ...
      attention: {
        overdueActions: inspectionsEnabled
      ...
      unavailableDomains: [
        ...(!inspectionsEnabled ? ['INSPECTIONS'] : []),
        ...(!incidentsEnabled ? ['INCIDENTS_AND_OBSERVATIONS'] : []),
        ...(!trainingEnabled ? ['TRAINING'] : []),
        ...(!ppeEnabled ? ['PPE'] : []),
      ],
      filterScope: {
        workCenterId: 'Todas las métricas con alcance por centro.',
        findingCategory:
          'Hallazgos, tendencia de hallazgos y series de metodología/riesgo residual.',
        observationCategory: 'Observaciones y su tendencia.',
        incidentEventType: 'Incidentes y su tendencia.',
        dateRange:
          'Hallazgos, observaciones, incidentes y series derivadas; no Plan, capacitación, EPP ni revisión profesional.',
      },
      trends: trend,
